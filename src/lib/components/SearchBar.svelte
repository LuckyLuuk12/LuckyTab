...
<script lang="ts">
  ...
  import googlemapsIcon from "$lib/assets/img/googlemaps.png?enhanced";
  ...
  import chatGPTIcon from "$lib/assets/img/chatgpt.webp?enhanced";
...
<form class="multi-search-container" on:submit|preventDefault={submitForm}>
  <div class="msc-row">
    <button
      ...
      on:click={() => providersVisible.update((v) => !v)}
    >
      {#if $selected}
